"""
批次填充 trade_amenities — 快速版 (無 bounded 模式)
每筆交易查 6 次 API，客戶端過濾距離
"""
import sys
import time
import json
...
# Faster: no bounded mode, filter client-side
REQUEST_INTERVAL = 0.5  # faster without bounded
last_request_time = 0

DIMENSIONS = [
    ("transit", "捷運站 MRT", 1000),
    ("school", "學校", 1500),
    ("hospital", "醫院", 1200),
    ("shop", "7-11 全家", 800),
    ("park", "公園 park", 1000),
    ("restaurant", "餐廳 restaurant", 800),
]
...
def search_pois(keyword, lat, lon, radius_m):
    """Fast search without bounded mode."""
    url = (
        f"{NOMINATIM_BASE}/search?format=json&q={urllib.parse.quote(keyword)}"
        f"&limit=10&countrycodes=TW&addressdetails=0"
    )
...
def main():
    ...
    for i, (tid, lat, lon) in enumerate(trades):
        ...
        if cache_key not in cache:
            cache[cache_key] = {}
            for cat, keyword, radius in DIMENSIONS:
                pois = search_pois(keyword, lat, lon, radius)
