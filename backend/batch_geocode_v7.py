...
# -*- coding: utf-8 -*-
"""
批次 Geocoding v7 — 最終優化版

核心改進:
- Server-side cursor 直接流式讀取 trades 表（不需 temp table）
- DISTINCT ON 取代 GROUP BY（走索引，不用全表排序）
- Producer thread 餵料 → Queue buffer → 24 workers 查詢 → Writer thread 寫入
- 完全解耦：DB 慢不影響 worker 速度
- 批次印進度（每 500 筆一次）

Usage:
    /opt/hermes/.venv/bin/python batch_geocode_v7.py
    /opt/hermes/.venv/bin/python batch_geocode_v7.py --test N
"""
import sys
import re
import time
...
print(f"⚡ v7: {GEO_WORKERS} workers, task_buf={TASK_Q_MAX}, result_buf={RESULT_Q_MAX}")
...
def query_photon(client, query, bbox=None, target_city="", road_part=""):
    """Query Photon API with optional bbox and city filtering."""
...
# ── 每個 Worker 獨立的 client 重建機制 ──
_client_last_rebuild = {}  # thread_name -> last rebuild time
_client_lock = threading.Lock()

def get_or_rebuild_client(thread_name):
...
# ── Photon 健康檢查 + 自動等待 ──
_photon_healthy = None  # Unknown initially — force first check
_photon_check_lock = threading.Lock()

def check_photon_health():
    """快速檢查 Photon 是否存活."""
    global _photon_healthy
    with _photon_check_lock:
        try:
            import httpx as _httpx
            _c = _httpx.Client(timeout=2.0)
            _resp = _c.get('http://localhost:2322/api?q=test')
            _data = _resp.json()
            _c.close()
            if _data.get('type') == 'FeatureCollection':
...
def wait_for_photon(max_wait=600, check_interval=10):
    """等待 Photon 恢復，最多等 max_wait 秒."""
    start = time.time()
...
def photon_or_wait():
    ...
    if _photon_healthy is None or not _photon_healthy:
        if check_photon_health():
            return True
        # 真的當機了，印出警告並等待恢復
        print(f"\n  ⚠️ Photon 無回應！等待恢復中... (每 {10} 秒檢查一次)")
        recovered = wait_for_photon(max_wait=600, check_interval=10)
        if recovered:
            print(f"  ✅ Photon 已恢復！")
            return True
        else:
            print(f"  ❌ Photon 600 秒內未恢復，此批次將有大量失敗")
            return False
    return True
...
def main():
    ...
    print(f"Avg speed: {total_unique/total_time:.1f} addr/s")
