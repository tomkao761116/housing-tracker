"""
批次填充 trade_amenities — 穩定版
- 支援 429 重試
- 使用 bounded viewbox 確保結果在範圍內
- 每筆交易查 6 次 API
"""
import sys
import time
...
REQUEST_INTERVAL = 1.0  # Nominatim policy: 1 req/sec
last_request_time = 0
...
def search_pois(keyword, lat, lon, radius_m, limit):
    """Search Nominatim with bounded viewbox + 429 retry."""
    ...
    for attempt in range(3):
        ...
        try:
            ...
            if "429" in str(e) or hasattr(e, 'code') and e.code == 429:
                ...
                print(f"  ⚠ 429 on '{keyword}', waiting {wait:.1f}s (attempt {attempt+1}/3)", flush=True)
                time.sleep(wait)
                continue
            else:
                print(f"  ⚠ Error on '{keyword}': {e}", flush=True)
                return []
